...
const profileData = [
  ...
  { icon: MdOutlineLocationOn, label: "Location", value: "Vietnam" },
  { icon: FaCode, label: "Coding", value: "Fullstack Developer" },
  { icon: LuBriefcaseBusiness, label: "Job", value: "Software Engineer" },
  { icon: FaRegHeart, label: "Relationship", value: "Business Analyst" },
  { icon: PiReadCvLogo, label: "Resume", value: "Available" },
  { icon: GrScorecard, label: "Score", value: "A+" },
];
  ...
  return (
    <div>
      <div className="flex flex-row justify-between items-center">
        ...
        <div className="pt-7">
          {profileData.slice(6).map((item, index) => (
            <div key={index} className="flex flex-row items-center space-x-2">
              <item.icon className="text-xl" />
              <span className="font-medium">{item.label}:</span>
              <span>{item.value}</span>
